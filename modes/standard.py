...
class Standard:
	...
	def data_in(self, data):
		...
		# SNAPSHOT CHANGE (Helix LT). Captured 2026-08-23 by pressing snapshots
		# 1, 2 and 3 on the device: each press emits two of these, one with
		# data[26] == 0x2a and one with 0x2e, both carrying the same 0-based
		# index at data[30]. set_snapshot() ignores a repeat of the current
		# value, so handling both is harmless.
		#
		# This must stay ahead of the generic 0x17/0xf0 handler below: the two
		# share their first 16 bytes, so the generic one would swallow it.
		elif self.helix_usb.my_byte_cmp(left=data, right=[0x17, 0x0, 0x0, 0x18, 0xf0, 0x3, 0x2, 0x10, 0x0, "XX", 0x0, 0x4, 0x9, 0x2, 0x0, 0x0, 0x0, 0x0, 0x4, 0x0, 0x7, 0x0, 0x0, 0x0, 0x82, 0x69, "XX", 0x6a, 0x81, 0x5c, "XX", 0x44], length=32) \
				and data[26] in (0x2a, 0x2e):
			# Same acknowledgement the generic 0x17 handler below sends. This
			# branch consumes a packet that would otherwise reach it, so the
			# reply has to be sent here too or the device stops being answered.
			out = OutPacket(data=[0x8, 0x0, 0x0, 0x18, 0x2, 0x10, 0xf0, 0x3, 0x0, "XX", 0x0, 0x8, 0x74, 0x77, 0x0, 0x0],
							delay=0.01)
			self.helix_usb.out_packet_to_endpoint_0x1(out)
			self.helix_usb.set_snapshot(data[30] + 1)
			return False
